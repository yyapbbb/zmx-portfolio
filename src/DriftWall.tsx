...
export const DRIFT_WALL_ITEMS: Array<{ image: string; title: string; href?: string }> = Array.from(
  { length: 15 },
  (_, i) => {
...
const DEFAULT_ITEMS = DRIFT_WALL_ITEMS
...
const DriftWall = ({
  ...
  const handleWheel = useCallback((e) => {
    ...
    for (let c = 0; c < offsetsRef.current.length; c++) {
      offsetsRef.current[c] = (offsetsRef.current[c] ?? 0) + delta
