...
class Media {
  ...
  createShader() {
    ...
    fallbackContext.fillStyle = '#262626'
    fallbackContext.fillRect(0, 0, 16, 12)
    fallbackContext.fillStyle = '#f0863f'
    fallbackContext.fillRect(4, 4, 8, 4)
  ...
  update(scroll) {
    ...
    const scrollOffset = ((scroll.current % total) + total) % total
    this.plane.position.x = this.x - scrollOffset
...
class App {
  ...
  onWheel(e) {
    ...
    const multiplier = mode === 1 ? 18 : mode === 2 ? 120 : 1
    const horizontal = e.deltaX || 0
    const vertical = e.deltaY || 0
    const dominant = Math.abs(horizontal) > Math.abs(vertical)
      ? horizontal
      : vertical
    const rawDelta = dominant || e.wheelDelta || e.detail
    const delta = Math.max(-140, Math.min(140, rawDelta * multiplier))
    this.scroll.step += delta > 0 ? 1 : -1
    this.scroll.target = this.scroll.step * this.medias[0].width
  }
  onKeyDown(e) {
    switch (e.key) {
      case 'ArrowRight':
  ...
  stepBy(direction) {
    if (!this.medias || !this.medias[0]) return
    this.scroll.step += direction
    this.scroll.target = this.scroll.step * this.medias[0].width
  }
  getItemIndexAt(clientX) {
    const rect = this.container.getBoundingClientRect()
    const nx = ((clientX - rect.left) / rect.width) * 2 - 1
    const worldX = (nx * this.viewport.width) / 2
    const total = this.medias[0].widthTotal
    const offset = ((this.scroll.current % total) + total) % total
    const width = this.medias[0].width
    let best = null
    let bestDist = Infinity

    this.medias.forEach((media, index) => {
      const center = media.x - offset
      const distance = Math.abs(worldX - center)
    ...
    return best % (this.mediasImages.length / 2)
  }
  onResize() {
    this.screen = {
  ...
  update() {
    ...
    this.lastTime = now
    this.scroll.velocity *= Math.exp(-dt / 0.22)
    this.scroll.target += this.scroll.velocity * dt * 60
    const k = 1 - Math.exp(-dt / Math.max(this.scroll.ease, 0.001))
    this.scroll.current = lerp(this.scroll.current, this.scroll.target, k)
    if (this.medias) {
...
type CircularGalleryProps = {
  ...
  scrollSpeed?: number
  scrollEase?: number
  onImageClick?: (index: number, item: { image: string; text: string }) => void
}
