...
type AccordionGalleryProps = {
  items?: { image: string; label: string; link?: string; alt?: string }[]
  defaultIndex?: number
  accentColor?: string
  overlayColor?: string
  textColor?: string
  ...
  grayscale?: boolean
  className?: string
  onItemClick?: (index: number, item: { image: string; label: string }) => void
}
...
function AccordionGallery({
  ...
  const handleWheel = (e) => {
    ...
    setActive((current) => (current + (delta > 0 ? 1 : -1) + count) % count)
  }

  const goTo = (offset) => {
    setActive((current) => (current + offset + count) % count)
  }
